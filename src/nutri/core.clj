;Frontend:(funcionando e sem tradução)
  ...
  (letfn [(tentar []
            ...
            (let [entrada (read-line)]
              (try
                ...
                (catch Exception _ (println "Entrada inválida. Digite um número.") ((tentar))))))] (tentar)))
  ...
  (let [nome (read-line)
    ...
    (if (seq alimentos)
      (let [a (escolher-alimento alimentos)
            ...
            data (letfn [(valida [] (print "Data (ddmmaaaa ou dd/mm/aaaa): ") (flush)
                                  (let [d (read-line)]
...
(defn adicionar-exercicio []
  (println "Exemplos: caminhada, corrida, natação, flexão, ski")
  (print "Digite o nome da atividade (em português ou inglês): ") (flush)
  (let [entrada (read-line)
        nome (traduzir-exercicio entrada)
    ...
    (if (= 200 (:status response))
      (let [sugestoes (:sugestoes corpo)]
        ...
        (run! (fn [[idx s]]
                (println (str (inc idx) ". " (:name s)
                              " | Calorias/hora: " (:calories_per_hour s))))
              (map-indexed vector sugestoes))
        (print "\nEscolha uma opção: ") (flush)
        (let [opcao (parse-long (read-line))]
          (if (and (>= opcao 1) (<= opcao (count sugestoes)))
            (let [atividade (:name (nth sugestoes (dec opcao)))
                  ...
                  data (letfn [(valida [] (print "Data (ddmmaaaa ou dd/mm/aaaa): ") (flush)
                                        (let [d (read-line)]
                                          (if (or (re-matches #"\d{8}" d)
                                            ...
                                            (do (println "Data inválida.") (recur))))) ]
                         (valida))
                  payload {:atividade atividade :duracao duracao :data data}
